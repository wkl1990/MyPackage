...
outlier_connectivity <- function(data, onetrait=NULL, sdout=2, label=FALSE){
...
#'
#' This function remove outliers based on hierarchical clustering.
#'
#' @param data Data matrix
...
outlier_hcluster <- function(data, traitNum=NULL, scale=FALSE, cutheight=50000, minSize=10){
	...
	if (!is.null(traitNum)){
		...
		WGCNA::plotDendroAndColors(hc, traitColors, groupLabels = names(traitNum), main = "Samples dendrogram and trait heatmap")
	}
	outliers
}
...
outlier_PCA <- function(data, trait=NULL, colour=NULL, label=FALSE, pcout=0.3){
	pca = prcomp(data)
